// SuperPy - Altered to print variables with greater precision.
...
int main(int argc,char** argv)
{
	...
	if(test>0)
	{
		...
		printf("BR(b->s gamma)\t\t\t%.6E\n",bsgamma_calculator(name));
		printf("delta0(B->K* gamma)\t\t%.6E\n\n",delta0_calculator(name));

		printf("BR(Bs->mu mu)\t\t\t%.6E\n",Bsmumu_calculator(name));
		printf("BR(Bs->mu mu)_untag\t\t%.6E\n",Bsmumu_untag_calculator(name));
		printf("BR(Bd->mu mu)\t\t\t%.6E\n\n",Bdmumu_calculator(name));
	
		printf("BR(B->K* mu mu)_low\t\t%.6E\n",BRobs_BKstarmumu_lowq2_calculator(name,obs));
		printf("AFB(B->K* mu mu)_low\t\t%.6E\n",obs[1]);
		printf("FL(B->K* mu mu)_low\t\t%.6E\n",obs[2]);
		printf("AT1(B->K* mu mu)_low\t\t%.6E\n",obs[4]);
		printf("AT2(B->K* mu mu)_low\t\t%.6E\n",obs[5]);
		printf("AT3(B->K* mu mu)_low\t\t%.6E\n",obs[6]);
		printf("AT4(B->K* mu mu)_low\t\t%.6E\n",obs[7]);
		printf("AT5(B->K* mu mu)_low\t\t%.6E\n",obs[8]);
		printf("AI(B->K* mu mu)_low\t\t%.6E\n\n",AI_BKstarmumu_lowq2_calculator(name));
	
		printf("BR(B->K* mu mu)_high\t\t%.6E\n",BRobs_BKstarmumu_highq2_calculator(name,obs));
		printf("AFB(B->K* mu mu)_high\t\t%.6E\n",obs[1]);
		printf("FL(B->K* mu mu)_high\t\t%.6E\n",obs[2]);
		printf("HT1(B->K* mu mu)_high\t\t%.6E\n",obs[9]);
		printf("HT2(B->K* mu mu)_high\t\t%.6E\n",obs[10]);
		printf("HT3(B->K* mu mu)_high\t\t%.6E\n",obs[11]);
		printf("AI(B->K* mu mu)_high\t\t%.6E\n\n",AI_BKstarmumu_highq2_calculator(name));

		printf("q0^2(AFB(B->K* mu mu))\t\t%.6E\n",A_BKstarmumu_zero_calculator(name));
		printf("q0^2(AI(B->K* mu mu))\t\t%.6E\n\n",AI_BKstarmumu_zero_calculator(name));


		printf("BR(B->Xs mu mu)_low\t\t%.6E\n",BRBXsmumu_lowq2_calculator(name));
		printf("BR(B->Xs mu mu)_high\t\t%.6E\n",BRBXsmumu_highq2_calculator(name));
		printf("q0^2(AFB(B->Xs mu mu)\t\t%.6E\n",A_BXsmumu_zero_calculator(name));
		printf("BR(B->Xs tau tau)_high\t\t%.6E\n\n",BRBXstautau_highq2_calculator(name));
	
		printf("BR(B->tau nu)\t\t\t%.6E\n",Btaunu_calculator(name));
      		printf("R(B->tau nu)\t\t\t%.6E\n",RBtaunu_calculator(name));
      		printf("BR(B->D tau nu)\t\t\t%.6E\n",BDtaunu_calculator(name));
      		printf("BR(B->D tau nu)/BR(B->D e nu)\t%.6E\n",BDtaunu_BDenu_calculator(name));
     		printf("BR(Ds->tau nu)\t\t\t%.6E\n",Dstaunu_calculator(name));
     		printf("BR(Ds->mu nu)\t\t\t%.6E\n",Dsmunu_calculator(name));
     		printf("BR(D->mu nu)\t\t\t%.6E\n",Dmunu_calculator(name));
      		printf("BR(K->mu nu)/BR(pi->mu nu)\t%.6E\n",Kmunu_pimunu_calculator(name));
     		printf("Rmu23(K->mu nu)\t\t\t%.6E\n\n",Rmu23_calculator(name));
