...
ax.tick_params(axis='x', which='minor', bottom='off') # No minor ticks for x-axis.
...
for key in particles:
	...
	row = NP.argmin(chisq) # Row number of minimum chi-squared.
        ...
        profchisq = OneDim.ProfileLike(x, chisq, nbins=bins).profchisq # Profiled chi-squared.
        bestfit = data[key][row] # Best-fitting mass.

        xc = OneDim.ProfileLike(x, chisq, nbins=bins).bins # Bin centers.
        ci = OneDim.ConfidenceIntervals(profchisq, xc, epsilon=AP.epsilon).confint # Confidence intervals.
...
plt.subplots_adjust(bottom=0.2,left=0.2) # Might be necessary to make more room.
