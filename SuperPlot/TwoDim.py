...
class ProfileLike:
    def __init__(self, paramx, paramy, chisq, nbins, bin_limits=None):
        ...
        for i in range(bin_numbersx.size):
            if bin_numbersx[i] == 0: bin_numbersx[i] = 1
            if bin_numbersx[i] == nbins + 1: bin_numbersx[i] = nbins
            if bin_numbersy[i] == 0: bin_numbersy[i] = 1
            if bin_numbersy[i] == nbins + 1: bin_numbersy[i] = nbins
