...
#else
{
  ...
  printf("\nDark matter candidate is '%s' with spin=%d/2  mass=%.6E\n",
  cdmName,       spin2, Mcdm); 
...
{ double SMbsg;
  ...
  printf("deltartho=%.6E\n",deltarho());
  printf("gmuon=%.6E\n", gmuon());
  printf("bsgnlo=%.6E ", bsgnlo(&SMbsg)); printf("( SM %.6E )\n",SMbsg);

/*
  if(callSuperIsoSLHA()==0)
  {
    double   bsg=slhaValFormat("FOBS",0., " 5 1  %lf 0 2 3 22"); 
    printf("superIsoBSG=%e\n",bsg); 
  }
*/    
...
#endif
     printf("Photon flux = %.6E[cm^2 s GeV]^{-1} for E=%.1f[GeV]\n",SpectdNdE(Etest, FluxA), Etest);       
#ifdef LoopGAMMA
     if(loopGamma(&vcs_gz,&vcs_gg)==0)
     {
         printf("Gamma  ray lines:\n");
         printf("E=%.6E[GeV]  vcs(Z,A)= %.6E[cm^3/s], flux=%.6E[cm^2 s]^{-1}\n",Mcdm-91.19*91.19/4/Mcdm,vcs_gz,
                               gammaFlux(fi,dfi,vcs_gz));  
         printf("E=%.6E[GeV]  vcs(A,A)= %.6E[cm^3/s], flux=%.6E[cm^2 s]^{-1}\n",Mcdm,vcs_gg, 
...
#endif
    printf("Positron flux  =  %.6E[cm^2 sr s GeV]^{-1} for E=%.1f[GeV] \n",
...
#endif
    printf("Antiproton flux  =  %.6E[cm^2 sr s GeV]^{-1} for E=%.1f[GeV] \n",
...
printf("\n======== Direct Detection ========\n");    
  ...
  printf("73Ge: Total number of events=%.6E /day/kg\n",nEvents);
  printf("Number of events in 10 - 50 KeV region=%.6E /day/kg\n",
  ...
  printf("131Xe: Total number of events=%.6E /day/kg\n",nEvents);
  printf("Number of events in 10 - 50 KeV region=%.6E /day/kg\n",
                                   cutRecoilResult(dNdE,10,50));                                   
#ifdef SHOWPLOTS
  ...
  printf("23Na: Total number of events=%.6E /day/kg\n",nEvents);
  printf("Number of events in 10 - 50 KeV region=%.6E /day/kg\n",
                                   cutRecoilResult(dNdE,10,50));                                   
#ifdef SHOWPLOTS
  ...
  printf("I127: Total number of events=%.6E /day/kg\n",nEvents);
  printf("Number of events in 10 - 50 KeV region=%.6E /day/kg\n",
                                   cutRecoilResult(dNdE,10,50));                                   
#ifdef SHOWPLOTS
...
printf(" e^+, e^- annihilation\n");
  ...
  { int ntot,l;
    ...
    { int err;
      ...
      else if(cs) printf("%-20.20s  %.6E [pb]\n",txt,cs); 
    }
  } 
/*
 { double stot=0;
