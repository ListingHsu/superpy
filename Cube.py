...
# the cube by writing a information file.
...
class countcalls(object):
    ...
    def __init__(self, f):
        self.__f = f
        self.__numcalls = -1 # Start count at -1, so that first call is zeroth.
...
def AddCube(cube, x, label, name):
    ...
    if AddCube.count() + 1 > MN.n_params:
       sys.exit('''Cannot add more parameters to the cube than the length of the MultiNest cube.
...
# This info file can be used for plot labels.\n''')
    ...
    for key in Priors.CMSSMModelTracker().param:
        info.write('''%s %s %s\n''' % (key, Priors.CMSSMModelTracker().param[key].type, Priors.CMSSMModelTracker().param[key].arg))
        
    # Write the likelihoods types and arguments.
    info.write('''# Likelihoods.\n''')
    for key in Likelihood.CMSSMConstraintTracker().constraint:
        info.write('''%s %s %s\n''' % (key, Likelihood.CMSSMConstraintTracker().constraint[key].type, Likelihood.CMSSMConstraintTracker().constraint[key].arg))
